...
const input = "w-full border border-white/20 bg-[var(--background-deep)]/45 px-4 py-3 text-white focus:border-[var(--accent)]";
...
export function CareersAdmin() {
  ...
  if (!session) return <div className="container-shell max-w-xl"><p className="eyebrow">Careers administration</p><h1 className="display mt-5 text-5xl">Secure Sign In.</h1><p className="mt-5 leading-7 text-white/60">Approved Axe Build administrators can request a one-time sign-in link.</p><form onSubmit={requestLink} className="industrial-panel mt-8 p-6"><label htmlFor="admin-email" className="mb-2 block font-bold">Work email</label><input id="admin-email" type="email" required value={email} onChange={(event) => setEmail(event.target.value)} className={input} /><button disabled={busy} className="mt-5 bg-[var(--accent)] px-5 py-3 font-black uppercase text-[var(--background-deep)] disabled:opacity-50">{busy ? "Sending…" : "Email Sign-In Link"}</button>{message ? <p role="status" className="mt-4 text-sm text-white/65">{message}</p> : null}</form></div>;
  ...
  return <div className="container-shell"><div className="flex flex-wrap items-end justify-between gap-5"><div><p className="eyebrow">Careers administration</p><h1 className="display mt-5 text-5xl sm:text-7xl">Manage Opportunities.</h1></div><button onClick={() => getBrowserSupabaseClient().auth.signOut()} className="border border-white/25 px-4 py-3 text-xs font-black uppercase">Sign Out</button></div>
    ...
    {!editor ? <>
      <section className="mt-12"><div className="flex flex-wrap items-center justify-between gap-4"><h2 className="text-2xl font-black uppercase">Open Postings</h2><button onClick={() => setEditor({ kind: "posting", content: { ...empty } })} className="bg-[var(--accent)] px-5 py-3 text-sm font-black uppercase text-[var(--background-deep)]">+ New Job Posting</button></div><PostingList postings={open} edit={(item) => setEditor({ kind: "posting", id: item.id, content: item, currentStatus: item.status })} pdf={downloadPdf} transition={(id, status) => action({ action: "transitionPosting", id, status })} /></section>
      ...
      <section className="mt-12"><div className="flex flex-wrap items-center justify-between gap-4"><h2 className="text-2xl font-black uppercase">Templates</h2><button onClick={() => setEditor({ kind: "template", templateName: "", content: { ...empty } })} className="bg-[var(--accent)] px-5 py-3 text-sm font-black uppercase text-[var(--background-deep)]">+ New Template</button></div><div className="mt-5 grid gap-3">{templates.length ? templates.map((item) => <article key={item.id} className="industrial-panel flex flex-wrap items-center justify-between gap-4 p-5"><div><p className="font-black uppercase">{item.templateName}</p><p className="mt-1 text-sm text-white/55">{item.jobTitle}</p></div><div className="flex flex-wrap gap-2"><button onClick={() => setEditor({ kind: "template", id: item.id, templateName: item.templateName, content: item })} className="border border-white/25 px-3 py-2 text-xs font-black uppercase">Edit Template</button><button onClick={() => action({ action: "createPostingFromTemplate", templateId: item.id })} className="border border-[var(--accent)] px-3 py-2 text-xs font-black uppercase text-[var(--accent)]">Create Posting From Template</button></div></article>) : <Empty label="No templates yet." />}</div></section>
    </> : null}
  </div>;
}
